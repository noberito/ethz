...
cd _data
%-----------------------------------
% FIND AND LOAD NT20 FILES
%-----------------------------------
listOf = dir('*.txt');
ii=1;
for i=1:numel(listOf)
str=listOf(i).name;
if (strfind(str,'NT20_'))
...
thickness =str2num(cell2mat(A{1}(7)));
widthInner =str2num(cell2mat(A{1}(8)));%10;%
widthOuter =str2num(cell2mat(A{1}(9))); %10;%
...
line(u1,Force,'LineStyle','none','Marker','.','MarkerSize',3,'MarkerEdgeColor','k'); hold on; 
pos=randi([ind-50,ind+50],1);
...
xlabel('Displacement [mm]','FontSize',18);
ylabel('Force [kN]','FontSize',18);
eval(['axis([0 ' num2str(xaxis(1)+0.1) ' 0 ' num2str(max(Force)+1) '])']);
yyaxis right
...
for i=1:1%size(LocStrain_longi,2)
yaxisax2(1)=max(yaxisax2(1),max(LocStrain_longi(:,i)));
plot(u1,LocStrain_longi(:,i),'.b','MarkerSize',4); hold on; 
...
fid=fopen(name,'w');
fprintf(fid,'Time[s],Displacement[mm],Force[kN],AxStrain_1,AxStrain_2,Thickness[mm],OuterWidth[mm],InnerWidth[mm]\n');  
fclose(fid);
...
clear a A* ind* fmax Force Fract* Loc* extenso* ...
    force u* Mat* mmperpix NominalSpeed thickness widthOuter widthInner ...
...
%------------------------------------------------------- 
cd _results
savefig(1,'_NT20_Force_Disp_LocAxStrain_all')
saveas(1,['_NT20_Force_Disp_LocAxStrain_all.png'],'png');
savefig(2,'_NT20_Time_Displacement_all');
saveas(2,['_NT20_Time_Displacement_all.png'],'png');
% savefig(3,'_NT20_LocAxStrain_Displacement');
% saveas(3,['_NT20_LocAxStrain_Displacement.png'],'png');
cd ..
hold off
%%
%-------------------------------------------------------
...
ii=1;
for i=1:numel(listOf)
if (strncmp(listOf(i).name,'NT20_',5)==1)
filenames{ii}=listOf(i).name;
ii=ii+1;
end
end
...
geometry=  name(1:4);
orientation= name(6:7);
testnumber = name(9);
plotname = [geometry '-' orientation '-' testnumber];
testname = [geometry '_' orientation '_' testnumber];    
nameori = [geometry '_' orientation];
testnumber = str2num(name(9));
...
eval(['widthInner(testnumber).' nameori '=A(1,8);']);  
...
try
eval(['area(testnumber).' nameori '=(widthInner(testnumber).' nameori ').*thickness(testnumber).' nameori ';']);
...
% DETERMINE ERROR
%for ii=1:2 %Hardcoded because numbering of tests is off 
for ii=1:ll  
...
saveas(1,['NT20_Force_Disp_normalized.png'],'png')
