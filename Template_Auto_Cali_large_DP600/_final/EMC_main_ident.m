...
%     % BASED ON SIMS
%     if str2num(ori)==00
%     xEMC  = [ 1.7	1.17	0.00*100	0.1]; % 00
%     elseif str2num(ori)==45
%     xEMC  = [ 1.8	1.17	0.00*100	0.1]; % 90
%     elseif str2num(ori)==90
%     xEMC  = [ 1.8	0.95	0.01*100	0.1]; % 90 
%     end
    
    % BASED ON EXP ONLY    
%     if str2num(ori)==00
%     xEMC  = [ 1.78	0.5	0.072*100	0.1]; % 00
%     elseif str2num(ori)==45
%     xEMC  = [ 1.765	0.5	0.085*100	0.1]; % 90
%     elseif str2num(ori)==90
%     xEMC  = [ 1.73	0.5	0.0825*100	0.1]; % 90 
%     end
% 
%     %-------------------AUTO------------------------------
    pv = [1.05 0.65 1.11];
...
%     fminsearchbnd
    options = optimset('Display','iter','MaxIter',500,'TolFun',1e-7, 'TolX',1e-7)
...
for lll=[31 101 146 191]
   try
    ...
    axis([-0.1 1.1*xmax 0.0 1.2*ymax])
    axis square;
    box on
        if lll == 31 
        ...
        elseif lll == 146 
        saveas(gcf,['_final_Triax_PEEQ_HC_45.png'],'png');
        savefig(lll,'_final_Triax_PEEQ_HC_45');
        elseif lll == 191 
...
for lll=[32 102 147 192]
    ...
    box on
        ...
        elseif lll == 147 
        saveas(gcf,['_final_Lode_PEEQ_HC_45.png'],'png');
        savefig(lll,'_final_Lode_PEEQ_HC_45');
        elseif lll == 192 
