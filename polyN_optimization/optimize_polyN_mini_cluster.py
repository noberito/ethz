...
def optiCoeff_polyN_mini(df, degree, weight_exp, weight_rval):
    ...
    def grad_J(a):
        ...
        grad[-2] = np.sum(weight_s * eq_ys * 2 * np.square(data[:,4]))
        grad[-1] = np.sum(weight_s * eq_ys * 2 * np.square(data[:,5]))
    ...
    opt = scipy.optimize.minimize(J, x0=a0, jac=grad_J, method="trust-constr", constraints=constraints, tol=1e-4, options={"verbose":3, "maxiter":10000})
    coeff_temp = opt.x
