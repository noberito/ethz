...
!C  VECTOR O MATERIAL PARAMETERS
    ...
    !C  LOOP COUNTERS
        INTEGER::k,K1,K2,NRK,KK,LL,MM,II,JJ
    ...
    !C  DEFINE COMPLIANCE (note that it outputs ENGINEERING shears)
        ...
        !C******************************************************	     
            ...
            !C      CALCULATE NEW VALUES ASSOCIATED WITH NEW STATE
                    ...
                    FZERO=F1*F1
                    !cDO K1=1,NTENS,1
                        !cTT=DEPBAR(k)*GYF(K1)-strainInc(k, K1)
                        !cDO K2=1,NTENS,1
                            !cTT=TT+SCOMP(k, K1,K2)*DSIGMA(k, K2)
                        !cEND DO
                        !cF2(K1)=TT
                        !cFZERO=FZERO+TT*TT
                    !cEND DO
...
!CCCCCC************** 2) HOMOGENEITY DEGREE = N (any degree)
    ...
    IF (ABS(SIGMA(2)) > MAX) THEN
        MAX = ABS(SIGMA(2))
    END IF

    IF (ABS(SIGMA(3)) > MAX) THEN
        MAX = ABS(SIGMA(3))
    END IF

    IF (ABS(SIGMA(4)) > MAX) THEN
        MAX = ABS(SIGMA(4))
    END IF

    IF (ABS(SIGMA(5)) > MAX) THEN
        MAX = ABS(SIGMA(5))
    END IF
    
    IF (ABS(SIGMA(6)) > MAX) THEN
        MAX = ABS(SIGMA(6))
    END IF
